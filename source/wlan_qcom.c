...
void httpGet(const char *hostname, const char *reqdata, char *respdata)
{
	...
	do
	{
		...
		if (isQcomError(status, "qcom_connect"))
			break;
		char request[512];
		sprintf(request,"GET https://%s/%s HTTP/1.0\r\nHost: %s\r\n\r\n", hostname, reqdata, hostname);
		...
		if (sent < 0)
			break;
		QCA_CONTEXT_STRUCT *enetCtx = wlan_get_context();
		status = (A_STATUS)t_select(enetCtx, sock, 2000);
		if (status == A_ERROR)
			break;
		char *recvBuf = NULL;
		...
		{
			recvBuf[recvLen] = 0;
			memcpy(respdata, recvBuf, recvLen+1);
		}
...
void httpDelete(const char *hostname, const char *reqdata, char *respdata)
{
	...
	do
	{
		...
		if (isQcomError(status, "qcom_connect"))
			break;
		char request[1024];
		sprintf(request,"DELETE https://%s/%s HTTP/1.0\r\nHost: %s\r\n\r\n", hostname, reqdata, hostname);
		int reqLen = strlen(request) + 1;
		// This is how you allocate a send buffer
		char *sendBuf = custom_alloc(reqLen);
		if (sendBuf == NULL)
...
void httpPost(const char *hostname, const char *reqdata, char *respdata)
{
	...
	do
	{
		...
		if (isQcomError(status, "qcom_connect"))
			break;
		char request[2048];
		sprintf(request,"POST https://%s/%s HTTP/1.0\r\nHost: %s\r\n\r\n", hostname, reqdata, hostname);
		int reqLen = strlen(request) + 1;
		// This is how you allocate a send buffer
		char *sendBuf = custom_alloc(reqLen);
		if (sendBuf == NULL)
