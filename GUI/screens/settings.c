...
static void slider_brightness_evt_handler(lv_obj_t * slider, lv_event_t event){
	if(event == LV_EVENT_VALUE_CHANGED){
		static char buf[4];
		snprintf(buf, 4, "%d", lv_slider_get_value(slider));
		lv_label_set_text(NULL, buf);
	}
}
...
static void spinboxred_plus_event_cb(lv_obj_t * btn, lv_event_t event){
	if(event == LV_EVENT_SHORT_CLICKED || event == LV_EVENT_LONG_PRESSED_REPEAT)
	{
		...
		GUI_RedTerm = lv_spinbox_get_value(spinbox_red);
	}
}
...
static void initLabels(void)
{
	...
	lv_obj_align(label_status, main_page, LV_ALIGN_IN_TOP_MID, 0, 20);
	...
	lv_obj_align(label_term, main_page, LV_ALIGN_IN_TOP_LEFT, 10, 50);
	lv_obj_add_style(label_term, LV_LABEL_PART_MAIN, &style_font20);

	label_termdays = lv_label_create(main_page, NULL);
	lv_label_set_text(label_termdays, STR_SETTINGS_DAYS_TO_END);
	lv_obj_align(label_termdays, main_page, LV_ALIGN_IN_TOP_RIGHT, -20, 50);
	lv_obj_add_style(label_termdays, LV_LABEL_PART_MAIN, &style_font20);

	label_redterm = lv_label_create(main_page, NULL);
	lv_label_set_text(label_redterm, STR_SETTINGS_LABELS_RED);
	lv_obj_align(label_redterm, label_term, LV_ALIGN_IN_TOP_LEFT, 10, 50);
	lv_obj_add_style(label_redterm, LV_LABEL_PART_MAIN, &style_redterm);

	label_yellowterm = lv_label_create(main_page, NULL);

	lv_obj_align(label_yellowterm, label_term, LV_ALIGN_IN_TOP_LEFT, 10, 90);
	lv_obj_add_style(label_yellowterm, LV_LABEL_PART_MAIN, &style_yellowterm);
	...
	lv_spinbox_set_value(spinbox_red, 3);
...
static void initNetwork(void)
{
	...
	lv_obj_align(network_label, main_page,  LV_ALIGN_CENTER, 0, 90);
...
lv_obj_t * SETTINGS_Init(void){
	...
	/*
	label_brightness = lv_label_create(screen, NULL);
		lv_obj_set_pos(label_brightness, 210, 20);
		lv_label_set_text(label_brightness, "Jasnosc");

	slider_brightness = lv_slider_create(screen, NULL);
	lv_obj_set_width(slider_brightness, 350);
	lv_obj_set_pos(slider_brightness, 60, 50);
	lv_obj_set_event_cb(slider_brightness, slider_brightness_evt_handler);
	lv_slider_set_range(slider_brightness, 0 , 100);


	label_slider_value = lv_label_create(screen, NULL);
	lv_label_set_text(label_slider_value, "0");
