...
uint32_t RTC_GetSecondFromRTC(RTC_Type *base){
	return RTC_GetSecondsTimerCount(base);
}


void RTC_SecondToDateTime(uint32_t sec,rtc_datetime_t *dateTime){
	RTC_ConvertSecondsToDatetime(sec,dateTime);
}
